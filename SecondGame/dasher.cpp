...
int main()
{
    ...
    for (int i = 0; i < sizeOfNebulae; i++)
    {
        ...
        nebulae[i].pos.y = windowDimention[1] - nebula.height / 8;
        nebulae[i].frame = 0;
        nebulae[i].runningTime = 0.0;
        nebulae[i].updateTime = 1.0 / 16.0;

        nebulae[i].pos.x = windowDimention[0] + 300 * i;
    }
    ...
    while (!WindowShouldClose())
    {
        ...
        if (isOnGround(scarfyData, windowDimention[1]))
        {
            velocity = 0;
            isInAir = false;
        }
        ...
        {
             DrawText("You Win!", windowDimention[0] / 4, windowDimention[1] / 2, 40, WHITE);
        }
        else
        {
            for (int i = 0; i < sizeOfNebulae; i++)
